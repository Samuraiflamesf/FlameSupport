        ...
        <div class="card flex-shrink-0 w-full max-w-sm shadow-2xl bg-neutral text-primary-content">
            <div class="card-body">
                ...
                <form action="{{ route('supports.update', $support->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    <input name="subject" type="text" placeholder="Assunto" value="{{ $support->subject }}" class="input input-bordered input-secondary input-bordered w-full" required>
                    <textarea name="body" cols="30" rows="5" class="textarea textarea-secondary  textarea-bordered textarea-md w-full mt-2" placeholder="Descricao" required>{{ $support->body }}</textarea>
                    <button type="submit">
